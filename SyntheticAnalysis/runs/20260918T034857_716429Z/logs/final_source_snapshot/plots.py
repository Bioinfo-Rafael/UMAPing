...
def truth_figures(run):
    ...
    handles=[Line2D([],[],marker='o',ls='',color=COLORS[s],label=name) for s,name in STRUCTURES.items()]
    ...
    fig.legend(handles=[Line2D([],[],marker='o',ls='',color=COLORS[s],label=name) for s,name in STRUCTURES.items()],loc='outside lower center',ncol=5,frameon=False)
