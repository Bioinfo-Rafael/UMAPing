...
def save(fig,base):
    ...
    fig.savefig(base.with_suffix('.png'),dpi=165);fig.savefig(base.with_suffix('.pdf'));plt.close(fig)
...
def truth_figures(run):
    ...
    for row,condition in enumerate(['disconnected','sparse_bridge']):
        ...
        _,d=neighbors(z,z[ref],16);density=np.log10(15/(np.pi*np.maximum(d[:,-1],1e-12)**2))
        for col in range(3):
            ax=axes[row,col]
            if col==0:
                ...
                c=df.progress if col==1 else density;im=ax.scatter(*z.T,s=3,c=c,cmap='viridis',vmin=0 if col==1 else None,vmax=1 if col==1 else None,rasterized=True)
                fig.colorbar(im,ax=ax,shrink=.65,label='Within-structure progress / ellipse x' if col==1 else 'log10(15 / pi r15^2), diagnostic')
            ax.set_aspect('equal');ax.set_xlim(-21,21);ax.set_ylim(-7,7);ax.set_title(condition+' | '+['structure','progress','local density'][col]);ax.set_xlabel('z0');ax.set_ylabel('z1')
        axes[row,0].legend(loc='lower center',bbox_to_anchor=(.5,-.32),ncol=3,markerscale=3,fontsize=8)
    fig.suptitle('Fixed geometry BEFORE training | 6,000 shared internal points + 480 bridge points',fontsize=14)
    ...
    for ax,condition in zip(axes,['disconnected','sparse_bridge']):
        ...
        item=audit[condition+'_reference'];ax.set_title(f'{condition}: {item["components"]} components, no flagged shortcuts');ax.set(xlim=(-21,21),ylim=(-7,7),xlabel='z0',ylabel='z1');ax.set_aspect('equal')
...
def panels(run,condition,seed,kind,number,subset=None,suffix=''):
    ...
    elif kind=='progress':norm=Normalize(0,1);cmap='viridis';clabel='A: arclength; B: within branch; C: normalized x; bridges: arclength'
    elif kind=='density':
